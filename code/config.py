...
VADER_SID = SentimentIntensityAnalyzer()  # Instantiated for sentiment analysis
SELECTED_FEATURES = ['volatility_SOLUSDT', 'sol_btc_corr', 'sol_eth_corr', 'close_SOLUSDT_lag1', 'close_BTCUSDT_lag1', 'close_ETHUSDT_lag1', 'volume_change_SOLUSDT', 'volatility_BTCUSDT', 'volume_change_BTCUSDT', 'momentum_SOLUSDT', 'close_SOLUSDT_lag30', 'close_BTCUSDT_lag30', 'close_ETHUSDT_lag30', 'vader_sentiment', 'log_return_SOL_lag1', 'log_return_SOL_lag7', 'log_return_SOL_lag30', 'sol_btc_ratio', 'sol_eth_ratio', 'sign_return_SOL_lag1', 'momentum_filter']
MODEL_PARAMS = {'n_estimators': 1000, 'learning_rate': 0.01, 'hidden_size': 128, 'num_layers': 3, 'max_depth': 5, 'num_leaves': 31, 'alpha': 0.1, 'lambda': 0.1}  # Tuned for better R2, ZPTAE, directional accuracy
OPTUNA_TRIALS = int(os.getenv('OPTUNA_TRIALS', 100))  # Increased for better tuning
USE_SYNTHETIC_DATA = os.getenv('USE_SYNTHETIC_DATA', 'True')  # Blend real/synthetic data