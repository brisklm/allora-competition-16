...
SELECTED_FEATURES = ['volatility_SOLUSDT', 'sol_btc_corr', 'sol_eth_corr', 'close_SOLUSDT_lag1', 'close_BTCUSDT_lag1', 'close_ETHUSDT_lag1', 'volume_change_SOLUSDT', 'volatility_BTCUSDT', 'volume_change_BTCUSDT', 'momentum_SOLUSDT', 'close_SOLUSDT_lag30', 'close_BTCUSDT_lag30', 'close_ETHUSDT_lag30', 'sentiment_score', 'sol_btc_ratio', 'log_return_SOL_lag1', 'sign_return_SOL_lag1', 'volatility_ETHUSDT', 'volume_change_ETHUSDT', 'momentum_filter']
MODEL_PARAMS = {'n_estimators': 1000, 'learning_rate': 0.01, 'max_depth': 5, 'num_leaves': 31, 'reg_alpha': 0.1, 'hidden_size': 128, 'num_layers': 3, 'dropout': 0.2}
OPTUNA_TRIALS = int(os.getenv('OPTUNA_TRIALS', 100))
USE_SYNTHETIC_DATA = os.getenv('USE_SYNTHETIC_DATA', 'True')
USE_BLENDED_DATA = True  # Blend real and synthetic
HANDLE_NANS = True  # Flag to handle NaNs and low variance in data processing